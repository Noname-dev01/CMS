...
> 최근 갱신: 2026-09-26(5차) — Remediation Plan PR 3(API 오류 계약과 안전한 500 진단, M-03) 구현 완료 반영. `/suggestRoadmap` → PR 3 선택 → `/plan-review-loop`(codex CLI 2라운드, v9·v10, ship — 1라운드에서 원 계획이 놓친 `HttpMediaTypeNotAcceptableException`(406) 및 API 전체 JSON Content-Type 보장·BindException 타입 변환 메시지 비노출 범위 확장 발견) → 사용자 구현 승인 → 구현·테스트까지 이번 세션에서 직접 수행: `GlobalApiExceptionHandler`에 400/405/415/406 handler 4종 신규 + 기존 12개 handler를 `jsonError()` 공통 조립으로 리팩터링해 JSON Content-Type 중앙 보장, `buildValidationMessage()`에 `isBindingFailure()` 분기로 타입 변환 메시지 비노출, catch-all에 안전한 진단 로그(method·라우트 패턴·예외 class·제한된 stack frame, 민감정보 미포함) 추가. 신규 테스트 21개(단위 13·통합 7·기존 파일 확장 2) + 전체 스위트 740개 통과(Docker Desktop 재기동 후 Testcontainers 포함). `fix/api-error-contract` 브랜치에서 작업 — **커밋·PR·머지는 아직**(`/code-review-loop` → `/commitPR` 단계 예정). 상세는 "실행 로드맵 — Remediation Plan" PR 3 참조.
> 이전 갱신: 2026-09-24(4차) — `/updateRoadmap` 사실확인: PR 2(ADMIN bootstrap·로그인 상태 분리, H-01)의 "커밋·PR·머지 미완료" 잔여 메모를 최신 사실로 갱신. 커밋 `eaaccb6` "보안: ADMIN bootstrap 기동 조건을 상태 allowlist로 확장 (감사 H-01) (#38)" 확인, `gh pr view 38` state=MERGED(mergedAt=2026-09-24T11:52:37Z), `gh pr checks 38` `test` pass(2m0s) 사실확인 후 반영. 상세는 "실행 로드맵 — Remediation Plan" PR 2 참조.
> 이전 갱신: 2026-09-24(3차) — Remediation Plan PR 2(ADMIN bootstrap·로그인 상태 분리, H-01) 구현 완료 반영. `/suggestRoadmap` → PR 2 선택 → `/plan-review-loop`(codex CLI 2라운드, ship) → 사용자 구현 승인 → 구현·테스트·실기 검증까지 이번 세션에서 직접 수행: `MemberRepository.existsByUserTypeAndStatusIn` 신규(기존 단일-ACTIVE 질의 대체), `AdminBootstrapLoader`의 존재 질의·재조회 흡수를 `{ACTIVE,LOCKED,PASSWORD_EXPIRED}` allowlist로 통일. 신규 테스트 30개(단위 15·Testcontainers 통합 15) + 전체 스위트 720개 통과, 실제 prod JAR+별도 MariaDB로 "LOCKED 관리자만 있어도 부트스트랩 변수 없이 기동" 실기 확인(사용자 dev 스택 미간섭). `security/admin-bootstrap-eligible-status` 브랜치에서 작업 — 이 시점 기준 커밋·PR·머지는 아직이었다(이후 4차 갱신에서 해소 반영).
...
- ~~**비밀번호 재설정 공개 API의 IP 기반 rate limit**: 계정별 60초 쿨다운·열거 방지는 이미 있음 — 실배포(⑤) 이후 노출 환경이 생기면 재평가.~~ → **해소됨** (`38ee3bc` #32, 2026-08-27): 비밀번호 재설정 API 2종 + `/notices/**`(GET·HEAD)에 토큰 버킷 기반 레이트리밋 도입 완료 — 상세는 상단 "실행 로드맵 — Top 3 (2026-07-29 선정)" ④ 참조.
...
### ④ 무인증 공개 엔드포인트 레이트리밋 도입 (Top 3 외 신규 발굴) — ✅ 완료 (2026-08-27 · `38ee3bc` #32)
- **유형**: 보안 / **선정 이유**: 2026-07-29 Top 3 선정 당시에는 없던 항목. ③ 완료로 Top 3 전 항목이 마감된 뒤, 실배포 인프라(nginx·TLS·호스팅)는 호스트·도메인 확정이라는 사용자 결정 대기 상태라 지금 착수할 수 없는 상황에서 코드 레벨로 바로 닫을 수 있는 방어로 신규 발의됨(`PLAN-public-endpoint-rate-limit.md` Context 참조). "선정에서 탈락한 후보"의 "비밀번호 재설정 공개 API의 IP 기반 rate limit"과 "후속 과제 — ② 공개 첨부 다운로드 완료 시 기록"의 자원 고갈 위험 완화를 함께 겨냥.
- **완료 근거**: `com.cms.config.ratelimit`(`Ticker`/`SystemTicker`·`BucketKey`·`RateLimitDecision`·`Bucket`·`RateLimitRule`·`RateLimitProperties`·`RateLimitConfigValidator`·`TokenBucketRateLimiter`(Caffeine 래퍼)·`RateLimitFilter`·`RateLimitFilterConfig`) 전부 실존 확인. `SecurityConfig.java`에 `addFilterAfter(rateLimitFilter, CsrfFilter.class)` 확인(CSRF 검증 실패 요청이 quota를 소비해 피해자 IP의 quota를 고갈시키는 교차 사이트 공격 방지). `CustomErrorController`에 429 분기 + 신규 `templates/error/429.html` 확인. `application.yml`에 `cms.rate-limit.*`(`/notices/**` GET·HEAD, 비밀번호 재설정 API 2종 POST 규칙) 확인. 신규 테스트 6클래스(`TokenBucketRateLimiterTest`(`FakeTicker` 기반 단위)·`RateLimitPropertiesValidationTest`·`RateLimitFilterTest`·`RateLimitResponseTest`·`RateLimitFilterContainerRegistrationTest`(`RANDOM_PORT`, 컨테이너 이중 등록 안 됨 확인)) + `PasswordResetControllerTest` 확장분 실존 확인. `./gradlew test` 656개 전체 통과(레이트리밋 비활성화 상태에서 기존 테스트 429 회귀 없음) 확인. `PLAN-public-endpoint-rate-limit.md`(v13, 적대적 리뷰 12라운드) "구현·검증 결과" 섹션에 완료 기준 18개 항목 전부 `[x]`(실기 Playwright+curl: 첨부 다운로드 GET/HEAD 경계에서 429+`Retry-After`·`error/429.html` 실제 렌더링, 비밀번호 재설정 API 운영 설정(5회) 그대로 6회째 JSON 429, CSRF 토큰 없는 반복 POST는 quota 미소비(401만 반복), `X-Forwarded-For` 위조로 우회 안 됨, `/actuator/health` 무제한 유지, 규칙 간 독립성, 컨테이너 필터 이중 등록 안 됨, 관리 화면 골든 패스 회귀 없음) 기록 확인. `/code-review-loop` 1라운드(codex, 지적 0건) 거침. PR #32 머지 확인(`gh pr view 32` state=MERGED, mergedAt=2026-08-27). 스키마 변경 없음, 신규 의존성 1건(Caffeine — v4에서 사용자 재승인, 자체 구현 admission/sweep이 3라운드 연속 동시성 결함을 낸 것을 근거로 채택).
- **실행 원본**: [`plan/PLAN-public-endpoint-rate-limit.md`](plan/PLAN-public-endpoint-rate-limit.md) (적대적 리뷰 12라운드 — 8~9라운드째부터 재확인만 반복돼 사용자 결정으로 루프 종료 후 구현 착수, 구현·검증 결과 기록됨)
- **목표**: `/notices/**`(GET·HEAD)·비밀번호 재설정 API 2종(POST)에 토큰 버킷 기반 최소 방어를 적용해 무제한 요청을 값싸게 차단한다. 정확한 유량 계약을 보장하는 게이트웨이가 아니며, fail-open(캐시 포화 시 개별 IP의 정확한 누적치가 보장되지 않을 수 있음)을 운영 위험으로 명시 수용.
- **완료 기준**: 계획서에 18개 항목으로 명시 (`./gradlew test` 통과 / 첨부 다운로드·비밀번호 재설정 API 각각 경계에서 429 확인 / CSRF 실패 요청이 quota를 소비하지 않음(교차 사이트 quota 고갈 방지, 쟁점 4) / `X-Forwarded-For` 위조로 우회 안 됨 / `/actuator/health` 무제한 / 컨테이너 필터 이중 등록 안 됨 / 잘못된 설정 시 기동 실패 등) — 전 항목 실기 검증 사실확인.
- **착수 게이트**: 신규 의존성 1건(Caffeine) 사용자 승인 완료(2026-08-12). 스키마 변경·`authorizeHttpRequests` 인가 규칙 변경 없음.

## 실행 로드맵 — Top 5 (2026-09-05 선정, 기술 감사 기반)

> 선정 근거: 외부 기술 감사 보고서(`docs/CMS-technical-audit-2026-09-05.md`, 기준 커밋 `cd8ec40` #33, `docs/verification-summary.json`·`docs/input-probe-result.txt` 부속)를 코드 대조로 재검증 — 아래 5개 항목은 실제 코드에서 직접 확인했다(추정 아님). 열린 이슈/PR 0건, 소스 TODO/FIXME 0건(vendor 제외) 재확인. Top 3(2026-07-29)·④(2026-08-27) 전 항목 완료 상태에서 신규 선정.
> 감사 원본의 "지금 당장 해야 할 Top 5" 순서(H-01+M-07 → H-02 → H-03+M-01 → M-02+M-03 → H-04)를 그대로 채택 — 방치 비용(보안·데이터 정합성)이 크고 구현 범위가 작은 항목을 앞에 두는 기존 선정 기준과 부합해 재조정하지 않았다.
> 감사의 "나중에 해도 되는 항목"(Redis 세션·분산 요청 제한, 파일 스트리밍/외부 오브젝트 스토리지, 첨부 악성코드 검사, 대규모 검색 인덱스, 마이크로서비스 전환)은 기존 "지속 항목"·"선정에서 탈락한 후보" 판단과 동일한 결론이라 신규 반영하지 않는다 — 조건(다중 인스턴스·외부 업로더 허용 등)이 바뀌면 재평가.
> 우선순위에서 밀린 감사 항목(M-04·M-05·M-06·M-08·L-01·L-02) 6건은 이 섹션 하단 표에 별도 기록.

### ① 관리자 비밀번호 검증 정책 통일 + 이메일 정규화·길이 정합 (감사 H-01·M-07) — ✅ 완료 (2026-09-22 · `f890195` #36)
- **유형**: 보안·데이터 무결성 / **선정 이유**: 진입점 4곳(생성·변경·재설정·부트스트랩) 중 하나(`AdminSignupRequest`)만 비밀번호 길이 검증이 아예 없고, BCrypt 72바이트 경계에서 DTO 검증은 통과하지만 인코딩 단계에서 미처리 예외가 나 500으로 샐 수 있다 — 구현 범위(Bean Validation 조정 위주)가 작은데 방치 비용은 크다.
- **완료 근거**: `com.cms.admin.member.dto.request.validation`에 `MaxUtf8Bytes`/`MaxUtf8BytesValidator`(BCrypt 72바이트, `CharsetEncoder`+`CodingErrorAction.REPORT`로 고립 서로게이트 거부)·`MinCodePoints`/`MinCodePointsValidator`(NIST 코드포인트 카운팅) 신규 확인. `AdminSignupRequest`·`AdminMyPasswordChangeRequest`·`PasswordResetConfirmRequest`·`AdminBootstrapCredentials` 4개 진입점 전부 `@MinCodePoints(15)`+`@MaxUtf8Bytes(72)` 동일 적용 확인, `AdminSignupRequest`에 `userId`(`@Size(max=50)`)·`userName`(`@Size(max=100)`)·`email`(`@Size(max=100)`) 상한 추가 확인. `com.cms.admin.member.service.EmailNormalizer`(공통 정규화 유틸, 정규화 후 길이·malformed UTF-16 재검증 포함) 신규 확인 — `AdminMemberService`(`createAdmin`·`updateMyInfo`·`updateAdminMember`)·`PasswordResetService`·`AdminBootstrapLoader` 전부 이 유틸 사용으로 전환 확인(기존 두 곳의 중복·Locale 불일치 정규화 제거). 신규 테스트 `PasswordFieldBoundaryTest`(9개, 4개 DTO 공통 경계 매트릭스)·`EmailNormalizerTest`(6개) + 기존 4개 테스트 파일에 경계값·정규화 테스트 12개 추가 확인. `SPRING_PROFILES_ACTIVE=dev ./gradlew test` 685개 전체 통과(신규 27개 순증) 확인, PR #36 머지 확인(`gh pr view 36` state=MERGED, mergedAt=2026-09-22). Playwright 실기 검증: 13자 비밀번호 제출 시 서버 400 메시지 노출·18자로 정상 생성·이메일 대소문자 정규화(`CaseTest@TEST.com`→`casetest@test.com`) 저장·재설정 화면 코드포인트 카운팅(이모지 15개 경계) 직접 실행 확인. **완료 기준 중 "이메일 앞뒤 공백 케이스" 1건은 대상 자체가 성립하지 않음**(문자 그대로 미충족이 아니라 범위 재정의) — 계획 단계에서 `@Email`이 앞뒤 공백 포함 이메일을 이미 400으로 차단함을 코드·테스트로 실측 확인해, 정규화 유틸에 도달하는 공백 케이스가 존재하지 않는다(`PLAN-password-policy-unification.md` 쟁점 5 참조). 대소문자 케이스는 정상 검증됨.
- **실행 원본**: [`plan/PLAN-password-policy-unification.md`](plan/PLAN-password-policy-unification.md) (적대적 리뷰 4라운드 — 1~3라운드 codex, 4라운드는 codex 사용량 한도로 자체 수행 — ship 판정, 구현·검증 결과 기록됨)
- **목표**: 관리자 생성·변경·재설정·부트스트랩 네 경로가 동일한 비밀번호 정책(최소 길이 상향, BCrypt 72바이트 상한을 400으로 명시 처리)을 적용하고, 생성 DTO의 길이 상한을 `Member` 엔티티 컬럼(50/100/100)과 맞추며, 이메일 정규화(트림·소문자화 Locale)를 서비스 간에 통일한다.
- **수정해야 할 정확한 파일** (실측 기준):
    - 수정: `src/main/java/com/cms/admin/member/dto/request/AdminSignupRequest.java`(`pwd`에 길이 검증 추가, `userId`/`userName`/`email`에 DB 컬럼 대응 `@Size` 추가)
    - 수정: `src/main/java/com/cms/admin/member/dto/request/AdminMyPasswordChangeRequest.java`·`PasswordResetConfirmRequest.java`(현재 `@Size(min=4,max=100)` — 72바이트 상한 처리 정책이 정해지면 함께 조정)
    - 수정: `src/main/java/com/cms/admin/member/AdminBootstrapCredentials.java`(정책 변경 시 주석의 정합성 근거도 함께 갱신)
    - 수정: `src/main/java/com/cms/admin/member/service/AdminMemberService.java:404`(`normalizeEmail` — 기본 Locale `toLowerCase()`), `src/main/java/com/cms/admin/member/service/PasswordResetService.java:311`(`Locale.ROOT` 사용) — 공통 유틸로 통합
    - 검토: `src/main/java/com/cms/common/api/GlobalApiExceptionHandler.java`(BCrypt `IllegalArgumentException`을 별도 400으로 매핑할지 결정 — M-03과 함께 처리하면 원인 로깅까지 한 번에 닫힌다)
    - 신규 테스트: 각 DTO의 1자/72바이트/73바이트/다국어(한글 등 멀티바이트) 경계, 51자 `userId`, 이메일 대소문자·공백 정규화 일관성
- **단계별 작업 순서**: `/plan-review-loop`로 최소 길이·72바이트 처리 방식 확정(사용자 승인) → 공통 이메일 정규화 유틸 추출 → DTO `@Size` 조정 → 예외 매핑 → 테스트 → 회귀(`./gradlew test`)
...
- **수정해야 할 정확한 파일** (실측 기준):
    ...
    - 검토: `src/main/java/com/cms/admin/member/domain/Member.java:102`(`updateInfo` — 이메일 변경 시 토큰 null 대입 로직 자체는 유지하되, 행 잠금 하에서 동작함을 전제로 재확인), `@DynamicUpdate`(1~17행 주석)와의 상호작용 재검토
    - 신규 테스트: 실 DB(Testcontainers) 기반 동시성 재현 — 자기 정보 변경과 재설정 토큰 발급이 겹치는 경합 시나리오, 이메일 변경 후 옛 주소로 발급된 토큰의 재설정 실행 실패 확인
- **단계별 작업 순서**: `/plan-review-loop` 계획 확정(잠금 전환 범위·트랜잭션 경계 재확인) → `updateMyInfo` 잠금 전환 → 동시성 통합 테스트 작성 → 관리자 수정 경로와의 동일 필드 경합도 함께 확인 → 회귀
- **완료 기준**: 이메일 변경 후 옛 메일함으로 발급된 재설정 토큰이 실패하는 MariaDB 경합 테스트 통과 / 자기 수정과 관리자 수정 간 동일 필드 경합에서 lost update 없음 / 기존 내 정보 수정 골든 패스 회귀 없음
- **착수 게이트**: 없음(기존에 이미 승인된 자기 정보 수정 기능의 결함 수정 — 인가 정책 변경 아님).

### ③ 감사 로그 신뢰성 강화 — IP 위조 차단 + 커밋 순서 보장 (감사 H-03·M-01)
- **유형**: 보안·운영 / **선정 이유**: 감사 증거인 `AdminActionLog`의 IP가 클라이언트가 보낸 헤더를 그대로 신뢰해 위조 가능하고, `@AfterReturning` + `REQUIRES_NEW` 조합은 원 트랜잭션 커밋 전에 SUCCESS를 별도 커밋할 수 있어 "감사 성공 = 실제 성공"이라는 전제가 흔들릴 수 있다. 두 항목 모두 감사 로그의 신뢰성이라는 같은 목표를 향한다.
- **목표**: 신뢰할 수 없는 프록시 헤더로 감사 IP가 결정되지 않고, 감사 SUCCESS 기록이 원 비즈니스 트랜잭션의 실제 커밋 성공과 어긋나지 않는 상태.
- **수정해야 할 정확한 파일** (실측 기준):
    - 수정: `src/main/java/com/cms/admin/log/aspect/AdminActionLogAspect.java:95`(`getClientIp` — 신뢰 프록시 여부 검증 없는 `X-FORWARDED-FOR`/`X-Real-IP` 우선 로직 교정), 45행 길이 제한 대비 검증된 형식만 저장
    - 참고: `src/main/java/com/cms/config/auth/VisitLoggingAuthenticationSuccessHandler.java:159`(동일한 헤더 우선 로직 — 정책을 통일할지 계획 단계에서 결정)
    - 검토: `src/main/java/com/cms/admin/log/aspect/AdminActionLogAspect.java`(`@AfterReturning` 시점), `src/main/java/com/cms/admin/log/service/AdminActionLogService.java:24`(`REQUIRES_NEW`) — `afterCommit` 콜백 또는 동일 트랜잭션 outbox 방식으로 SUCCESS 확정 시점 재설계
    - 신규 테스트: 실제 Spring 프록시·트랜잭션 환경에서 커밋 직전 실패를 주입해 로그 결과 확인, 위조·과도하게 긴 IP 헤더에도 감사 행이 보존되는지 확인
- **단계별 작업 순서**: `/plan-review-loop` 계획 확정(IP 신뢰 경계 정책·SUCCESS 확정 시점 설계) → IP 결정 로직 교정 → 감사 저장 실패 카운터/알림 추가 → SUCCESS 확정 시점 재설계 → 커밋 실패 주입 테스트 → 회귀
- **완료 기준**: 위조·긴 헤더에도 감사 행의 IP 또는 저장 성공이 영향받지 않음 / 롤백된 작업이 SUCCESS로 남지 않음(실제 트랜잭션 주입 테스트로 확인) / 감사 저장 실패가 별도로 관측 가능
- **착수 게이트**: 프록시 헤더 신뢰 정책 변경은 실배포 인프라(리버스 프록시) 확정과 맞물릴 수 있어 계획 단계에서 확인.

### ④ 운영 가시성 확보 — SMTP 타임아웃/큐 상한 + 서버 오류 로깅 (감사 M-02·M-03)
- **유형**: 운영·가용성 / **선정 이유**: `application-prod.yml`에 SMTP 타임아웃이 없고 발송용 `TaskExecutor`에 큐 상한이 없어 SMTP 무응답 시 스레드가 무한정 점유될 수 있다. `GlobalApiExceptionHandler.handleException`은 500 응답만 만들고 원인을 전혀 기록하지 않아 이 문제를 포함한 모든 미분류 예외의 진단이 불가능하다 — 둘 다 운영 관측 공백이라는 같은 문제.
- **목표**: SMTP 연결 지연이 무한정 스레드를 점유하지 않고, 어떤 500 오류든 서버 로그에서 원인을 추적할 수 있는 상태.
- **수정해야 할 정확한 파일** (실측 기준):
    - 수정: `src/main/resources/application-prod.yml:13`(`mail:` 블록에 connection/read/write timeout 추가)
    - 검토: `src/main/java/com/cms/admin/member/service/PasswordResetService.java:132`(`mailExecutor` — 전용 executor 설정에 동시 실행 수·큐 크기 상한 추가)
    - 수정: `src/main/java/com/cms/common/api/GlobalApiExceptionHandler.java:366`(`handleException` — 요청 상관 ID·라우트·예외 유형을 서버 로그에 기록, 요청 본문·비밀번호·토큰은 수집하지 않음)
    - 신규 테스트: SMTP 무응답 상황(테스트 더블)에서 작업 수·최대 지연 제한 확인, 의도적 예외 발생 시 응답 ID로 로그 추적 가능·민감정보 미포함 확인
- **단계별 작업 순서**: `/plan-review-loop` 계획 확정(타임아웃 값·큐 크기 결정) → application-prod.yml 타임아웃 추가 → 전용 executor 큐 상한 설정 → 예외 핸들러 로깅 추가(민감정보 제거 확인) → 큐 포화/메일 실패 알림 여부 결정 → 테스트 → 회귀
- **완료 기준**: SMTP 무응답 테스트에서 작업 수·대기 시간이 제한됨 / 큐 포화 시 계정 열거 방지 균일 응답은 유지 / 의도적 내부 오류가 응답 ID로 서버 로그에서 추적되고 민감정보 미포함 확인
- **착수 게이트**: 없음(설정·로깅 추가 위주, 인가 정책 변경 없음).

### ⑤ 백업 시점 정합성 확보 (감사 H-04)
- **유형**: 운영·복구 / **선정 이유**: 기존 "후속 과제 — ① prod 프로파일 완료 시 발견"에 이미 기록된 "오프사이트 백업 미포함"과는 다른 새 문제 — `prod-backup.sh`가 DB 덤프 후 파일 볼륨을 압축해 **같은 백업 세트 안에서도 DB와 파일의 시점이 어긋날 수 있다**(DB 덤프 뒤 첨부 삭제가 커밋되면 백업 DB는 옛 파일을 참조하지만 아카이브엔 없음). 실데이터 운영 전 조건부 P1 — 순수 로컬 학습 단계면 배포 준비 시 처리.
- **목표**: 백업 한 세트 안의 DB 스냅샷과 파일 아카이브가 항상 같은 시점을 참조하고, 복구 검증이 체크섬뿐 아니라 참조 파일의 존재·해시·애플리케이션 조회까지 확인한다.
- **수정해야 할 정확한 파일** (실측 기준):
    - 수정: `scripts/prod-backup.sh:98`(DB 덤프와 파일 볼륨 압축의 시점 원자성 확보 — 소규모 운영 전제로 쓰기 앱 정지 백업을 기본 절차로 전환하는 방안 우선 검토)
    - 수정: `docs/deployment.md`(시점 불일치·오프사이트 부재는 이미 정확히 문서화됨 — 정지 백업 절차·RPO/RTO 확정 후 갱신)
    - 참고: `scripts/prod-restore.sh:192`(복구 검증에 DB 참조 파일 전체 존재·해시·애플리케이션 조회 단계 추가 검토)
- **단계별 작업 순서**: `/plan-review-loop` 계획 확정(정지 백업 여부·RPO/RTO 결정 — 실데이터 운영 계획이 아직 없다면 이 단계에서 범위를 "격리 환경 검증"으로 좁힐지 사용자와 확인) → 백업 스크립트에 정합성 보장 로직 추가 → 복구 검증 스크립트에 참조 정합성 확인 단계 추가 → 격리 환경 테스트 → 문서 갱신
- **완료 기준**: 격리 환경에서 DB 복구 후 모든 참조 파일의 존재·해시·권한과 애플리케이션 조회까지 확인 / 백업 실패 시 알림 경로 존재 / RPO/RTO 정의 문서화
- **착수 게이트**: 실데이터 운영 여부에 따라 범위가 달라짐 — 순수 로컬 학습 단계라면 원격 인프라 구축 없이 격리 백업/복구 검증부터 시작할지 사용자 확인.

### 우선순위에서 밀린 감사 항목 (다음 갱신 때 재평가)

| 항목 | 내용 | 미룬 이유 | 다시 착수할 조건 |
|---|---|---|---|
| M-04 | `prod-up.sh`/`prod-restore.sh`가 health 200+3초 대기로 완료 판정 — 가변 시간 러너(`ProfileImageMigrationRunner` 등)의 실제 준비 완료와 어긋날 수 있음 | 실배포 자동화 전에만 유효한 리스크, 현재 로컬 학습 단계 | 실배포 자동화·CD 파이프라인 구축 시 |
| M-05 | `AppConfig`의 KST `Clock`과 별개로 `Member`·`AdminActionLogService`·`NoticeAttachmentService`가 `LocalDateTime.now()` 직접 호출 — JVM이 UTC면 시각이 9시간 어긋날 수 있음 | 현재 컨테이너·로컬 모두 JVM 시간대를 KST로 맞춰 운용 중이라 즉시 영향 없음 | prod 이미지·Compose의 JVM 시간대가 명시적으로 고정되지 않은 채 실배포될 때 |
| M-06 | CI가 prod 이미지 기동·백업복구·의존성 스캔을 검증하지 않음, Dockerfile/Compose가 가변 태그(`mariadb:10.11`) 사용 | 테스트 커버리지 자체는 충분(656개) — CI 확장은 실배포 임박 시 레버리지가 더 큼 | 실배포 준비 단계 진입 시 |
| M-08 | `SecurityConfig` 마지막 규칙이 `anyRequest().permitAll()` — 미분류 URL 기본 공개 | 현재 `/admin/**`가 먼저 보호되고 신규 엔드포인트 추가 시 매번 명시적으로 규칙을 넣어온 이력(승인 이력 참조) — 즉각적인 무인증 노출은 발견되지 않음 | 기본 거부 전환은 인가 정책 전면 변경이라 별도 계획·사용자 협의 필요 시점에 |
...
> `remediation-plan.md` 상태(문서 자체 명시, 2026-09-24 작성 시점 기준): **PR 1·PR 2 구현·범위 내 검증 완료 / PR 3~6 구현 미착수**. H-01 A안 정책은 승인됐으며(2026-09-23), 사용자는 PR 1·PR 2 구현을 각각 승인해 순서대로 완료했다 — PR 3~6은 착수 전 별도 승인 필요. **PR 2는 이후 커밋 `eaaccb6` #38로 커밋·PR·머지까지 완료됨(2026-09-24, 아래 PR 2 항목 참조) — `remediation-plan.md` 문서 자체의 "커밋·PR 처리는 별도" 문구는 이 갱신 시점 기준으로는 이미 해소된 상태다.**
...
### PR 2 — ADMIN bootstrap과 로그인 상태 분리 (H-01) — ✅ 완료 (2026-09-24 · `eaaccb6` #38)
...
### PR 3 — API 오류 계약과 안전한 500 진단 (M-03) — ✅ 완료 (2026-09-26, `fix/api-error-contract` 브랜치 · 커밋·PR·머지 전)
...
- **완료 근거**: `GlobalApiExceptionHandler`에 `@ExceptionHandler` 4종(`MethodArgumentTypeMismatchException`→400, `HttpRequestMethodNotSupportedException`→405+`Allow`헤더, `HttpMediaTypeNotSupportedException`→415, `HttpMediaTypeNotAcceptableException`→406 — 계획 리뷰 1라운드에서 원 계획이 놓친 예외로 신규 발견돼 추가) 실존 확인. 기존 12개+신규 4개 handler 전부 `jsonError(status, path, code, message)` 공통 조립 메서드를 거쳐 `Content-Type: application/json`을 중앙에서 보장함을 코드에서 확인(계획 리뷰 2라운드 확인 — `@ExceptionHandler(produces=...)`로 handler 선택 자체를 제한하는 대체 방식은 `Accept: text/html`에서 handler 미선택 문제를 만들어 미채택). `buildValidationMessage()`가 `FieldError.isBindingFailure()`로 타입 변환 실패와 일반 Bean Validation 실패를 구분해, 타입 변환 실패 필드 오류만 고정 문구로 대체(일반 문구는 유지)함을 코드에서 확인. catch-all(`handleException`)이 method·라우트 패턴(`HandlerMapping.BEST_MATCHING_PATTERN_ATTRIBUTE`, 없으면 `"unmatched"`)·예외 class·제한된 stack frame만 로그에 남기고 Throwable 인스턴스를 로거에 직접 넘기지 않음을 확인(message/cause 체인 자동 노출 방지). 신규 테스트 `GlobalApiExceptionHandlerTest`(순수 단위 13개: ERROR 이벤트 정확히 1개·라우트 패턴 fallback·민감정보 미노출·400/405(Allow 유/무)/415/406 매핑·BindException 대체/유지) + `ApiErrorContractIntegrationTest`(실제 `SecurityConfig` 포함 슬라이스 7개: 400/405+Allow/415/406/500/`Accept: text/html` 회귀/미인증 401) 전부 통과 확인. 기존 `MenuControllerTest`(경로 변수 타입 불일치 400 회귀)·`AdminMemberControllerTest`(검색 열거형 필드 입력 표식 비노출) 각 1건 추가 확인. `SPRING_PROFILES_ACTIVE=dev ./gradlew test` 전체 740개 통과(신규 21개 순증, 실패·오류 0, Docker Desktop 재기동 후 Testcontainers 포함) 확인. 계획 리뷰(codex CLI 2라운드, ship 판정) 거침. `docs/troubleshooting.md`(애플리케이션/런타임 카테고리 신규 항목)·`.claude/skills/api-conventions/SKILL.md`(상태 코드 표에 405/406/415·JSON Content-Type 보장·BindException 비노출 반영) 문서 동기화 확인. `adversarial-review/remediation-plan.md` "PR 3 실행 기록"에 상세 기록. **커밋·PR 생성·머지는 미완료** — `/code-review-loop` → `/commitPR` 단계 예정.
- **목표**: `MethodArgumentTypeMismatchException`→400, `HttpRequestMethodNotSupportedException`→405(`Allow` 헤더 포함), `HttpMediaTypeNotSupportedException`→415, `HttpMediaTypeNotAcceptableException`→406(계획 리뷰 반영, 원 계획 대비 확장)으로 좁게 매핑하고, 나머지 예상 밖 예외는 여전히 500이되 method·라우트 패턴·예외 클래스·제한된 스택 프레임을 서버 로그에 남긴다(원시 URI/쿼리/바디/헤더/쿠키/비밀번호/토큰/예외 메시지는 로그에 넣지 않음). API 분기 전체의 JSON Content-Type 보장과 BindException 타입 변환 메시지 비노출도 계획 리뷰로 범위에 추가됐다.
...
- **완료 기준**(계획서 명시, 계획 리뷰로 확장): 전체 오류 행렬(406 포함) 통과 / Security 우선순위·HTML 오류 페이지 회귀 없음 / API 분기 전체 JSON Content-Type 보장 / BindException 타입 변환 메시지 비노출 / 안전한 진단 로그(민감정보 미포함) 검증 통과 / 새 예외 아키텍처·의존성·migration 없음 — 전 항목 사실확인.
- **착수 게이트**: 없음(설정·예외 매핑 위주, 인가 정책 변경 없음).
...
- **무인증 다운로드 경로의 자원 고갈 위험(명시적 수용)**: `/notices/{id}/attachments/{attachmentId}`는 파일을 `byte[]`로 전량 로딩하며(HEAD도 GET과 동일하게 서비스 진입·전체 로딩), 동시·반복 요청 제한이 없다. 파일당 10MB·공지당 5개 상한은 한 건당 비용만 제한한다. 소규모 운영을 전제로 이번 범위에서 수용한 위험이며(`PLAN-public-notice-attachment.md` 리스크 표), 실제 공개 트래픽·적대적 접근이 예상되면 `InputStreamResource` 스트리밍 전환·레이트리밋 도입을 재검토한다. **(2026-08-27 부분 완화, `38ee3bc` #32)**: `cms.rate-limit`로 첨부 다운로드 경로(GET·HEAD)에 IP당 요청 수 상한이 추가돼 반복 요청에 의한 자원 고갈 위험이 완화됐다 — 단 fail-open(캐시 포화 시 IP별 정확한 누적치 미보장)을 수용한 최소 방어이며, 파일당 `byte[]` 전량 로딩 자체(요청 1건당 비용)는 그대로라 `InputStreamResource` 스트리밍 전환 여부는 여전히 별도 재평가 대상이다.
...
**2026-09-24: 별도의 독립 재검증 감사(`deploy-check-2026-09-23.md`)·remediation 실행 계획(`remediation-plan.md`)을 사용자 요청으로 로드맵에 신규 편입.** Top 5(2026-09-05)와는 다른 감사 사이클로 발견된 C-01(High)·H-01(High)·M-01~M-06(Medium/Low, M-06은 철회) 8건을 PR 1~6 경계로 추적한다 — 상세는 "실행 로드맵 — Remediation Plan (2026-09-23 독립 검증 기반)" 참조. **PR 1(관리자 상세 저장형 마크업 렌더링 결함 수정, C-01)은 이미 완료됐다(2026-09-24 · `9294af5` #37).** **이후 PR 2(ADMIN bootstrap·로그인 상태 분리, H-01)도 완료됐다(2026-09-24 · `eaaccb6` #38), PR 3(API 오류 계약과 안전한 500 진단, M-03)도 구현·테스트까지 완료됐다(2026-09-26, `fix/api-error-contract` 브랜치 — 커밋·PR·머지는 아직)** — 남은 미완료는 PR 4~6(메뉴 행 잠금 일관화 M-04, SMTP 타임아웃 M-02, 백업/ingress 계약 문서화 M-01·M-05)이며, 계획서 자체가 이들을 "구현 미착수"로 명시한다. 다음 작업 선택은 `/suggestRoadmap`.
