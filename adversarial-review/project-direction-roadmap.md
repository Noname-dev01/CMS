...
> 최근 갱신: 2026-08-12 — `ProfileImageMigrationRunnerIntegrationTest`(Testcontainers) 추가로 "후속 과제 — ③" 2번 항목 완료 반영 (`ca6446f` #29, PR MERGED·CI test pass 사실확인) — 상세는 하단 "후속 과제 — ③ 프로필 이미지 이관 완료 시 발견" 참조. 남은 미해소는 1번 항목(실 레거시 데이터 이관 Playwright 골든 패스)뿐이다.
